...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="rounded-lg p-3 text-xs bg-white" style={{ border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
      ...
      <p className="text-red-500 font-semibold">risk_score: {d.risk_score?.toFixed(2)}</p>
    </div>
  )
}
...
const ScatterTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="rounded-lg p-3 text-xs bg-white" style={{ border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
      <p className="font-semibold text-slate-700">{getKsicName(d.ksic_code)} ({d.ksic_code})</p>
      <p className="text-slate-500">risk_score: {d.x?.toFixed(2)}</p>
      <p className="text-slate-500">최근 90일: {d.y}건</p>
      <p className="text-slate-500">총 발의: {d.z?.toLocaleString()}건</p>
    </div>
  )
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: '최고 risk_score', value: maxRisk.toFixed(2), color: '#ef4444' },
          { label: '최근 90일 최다 발의', value: maxRecent + '건', color: '#f59e0b' },
          { label: '규제법안 있는 산업', value: l2.filter(s => s.reg_count > 0).length + '개', color: '#a855f7' },
        ].map(card => (
          <div key={card.label} className="rounded-xl p-4 text-center bg-white" style={{ border: '1px solid #e2e8f0' }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl p-5 bg-white" style={{ border: '1px solid #e2e8f0' }}>
          <h3 className="text-sm font-semibold text-slate-700 mb-1">🔴 규제 위험 TOP 20</h3>
          <p className="text-xs text-slate-400 mb-4">risk_score = 규제법안수 × 규제법안가결률 / 100</p>
        ...
        {/* 최근 90일 TOP 20 */}
        <div className="rounded-xl p-5 bg-white" style={{ border: '1px solid #e2e8f0' }}>
          <h3 className="text-sm font-semibold text-slate-700 mb-1">📈 최근 90일 입법 활성 TOP 20</h3>
          <p className="text-xs text-slate-400 mb-4">최근 3개월간 발의된 법안 수</p>
          <ResponsiveContainer width="100%" height={360}>
      ...
      {/* 산점도 */}
      <div className="rounded-xl p-5 bg-white" style={{ border: '1px solid #e2e8f0' }}>
        <h3 className="text-sm font-semibold text-slate-700 mb-1">🔵 규제 위험 vs 입법 활성도</h3>
        <p className="text-xs text-slate-400 mb-4">오른쪽 위로 갈수록 규제도 강하고 최근에도 법안이 많이 발의됨 → 주목 필요</p>
        <ResponsiveContainer width="100%" height={360}>
          <ScatterChart>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            <XAxis type="number" dataKey="x" name="risk_score" tick={{ fill: '#94a3b8', fontSize: 11 }}>
              <Label value="risk_score →" fill="#94a3b8" fontSize={11} position="insideBottomRight" offset={-10} />
            </XAxis>
            <YAxis type="number" dataKey="y" name="최근90일" tick={{ fill: '#94a3b8', fontSize: 11 }}>
              <Label value="최근90일 발의" fill="#94a3b8" fontSize={11} angle={-90} position="insideTopLeft" />
            </YAxis>
